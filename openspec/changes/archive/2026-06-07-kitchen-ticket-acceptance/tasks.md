...
- [x] 1. Add `reject()` method to `KitchenTicket` domain model
...
- [x] 2. Add outbox event builders and new commands for accept/reject
  - [x] 2.1 Add `kitchen_ticket_accepted_event()` and `kitchen_ticket_rejected_event()` builder functions to `services/kitchen-service/src/kitchen_service/application/outbox.py`
  ...
  - [x] 2.2 Add `AcceptKitchenTicketCommand` and `RejectKitchenTicketCommand` dataclasses to `services/kitchen-service/src/kitchen_service/application/commands.py`
    - `AcceptKitchenTicketCommand(ticket_id: UUID)`
    - `RejectKitchenTicketCommand(ticket_id: UUID, rejection_reason: str | None = None)`
    - _Requirements: 1.1, 2.1_

  - [x]* 2.3 Write property test for outbox event payload completeness (Property 6)
...
- [x] 3. Fix `accept_ticket()` and add `reject_ticket()` in application service
  - [x] 3.1 Fix `accept_ticket()` in `services/kitchen-service/src/kitchen_service/application/tickets.py` to write to the outbox
  ...
  - [x] 3.2 Add `reject_ticket(ticket_id, rejection_reason=None)` method to `KitchenTicketApplicationService`
  ...
  - [x]* 3.3 Write property test for accept idempotency (Property 1)
  ...
  - [x]* 3.4 Write property test for reject idempotency (Property 2)
  ...
  - [x]* 3.5 Write property test for invalid transitions raise error (Property 3)
  ...
  - [x]* 3.6 Write property test for accept producing exactly one outbox event (Property 4)
  ...
  - [x]* 3.7 Write property test for reject producing exactly one outbox event (Property 5)
...
- [x] 4. Add unit tests for kitchen-service accept/reject application service
  - Add to `services/kitchen-service/src/tests/test_kitchen_tickets.py` (extend existing file):
  - [x]* 4.1 Write unit tests for `accept_ticket()` outbox behavior and `reject_ticket()`
...
- [x] 5. Checkpoint — Ensure all kitchen-service unit and property tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [x] 6. Add `POST /{ticket_id}/accept` fix and `POST /{ticket_id}/reject` endpoint to kitchen-service API
  - In `services/kitchen-service/src/kitchen_service/api/routes/tickets.py`:
  - [x] 6.1 Update the existing `/accept` route's 409 error response body to include `current_status` and `target_status` fields (access via `exc.current` and `exc.target`)
    - _Requirements: 1.4_

  - [x] 6.2 Add `POST /{ticket_id}/reject` route using `service.reject_ticket(ticket_id)` with the same 404/409 pattern; 409 body must include `current_status` and `target_status`
    - _Requirements: 2.1, 2.2, 2.4_

- [x] 7. Add `PREPARING` to `Order` domain and extend `cancel()`
  - In `services/order-service/src/order_service/domain/models.py`:
  - [x] 7.1 Add `PREPARING = "PREPARING"` to `OrderStatus` StrEnum
    - _Requirements: 8.1_

  - [x] 7.2 Add `begin_preparing()` method to `Order`
  ...
  - [x] 7.3 Extend `cancel()` to accept `PREPARING` as a valid source status
    - Change the guard from `{PENDING, APPROVED}` to `{PENDING, APPROVED, PREPARING}`
    - _Requirements: 7.2, 8.4_

  - [x]* 7.4 Write property test for `begin_preparing` state machine correctness (Property 7)
  ...
  - [x]* 7.5 Write property test for `cancel()` accepting PREPARING source (Property 8)
...
- [x] 8. Add unit tests for `Order` domain changes
  - Add to `services/order-service/src/tests/test_place_order.py` (extend existing file):
  - [x]* 8.1 Write unit tests for `begin_preparing()` and extended `cancel()`
...
- [x] 9. Add `begin_preparing_order()` and `cancel_order()` to `OrderLifecycleApplicationService`
  - In `services/order-service/src/order_service/application/lifecycle.py`:
  - [x] 9.1 Add `begin_preparing_order(order_id)` method following the same pattern as `approve_order()`
    - Return `None` if order not found; call `order.begin_preparing()`, save, commit
    - _Requirements: 6.1, 6.2_

  - [x] 9.2 Add `cancel_order(order_id)` method following the same pattern
    - Return `None` if order not found; call `order.cancel()`, save, commit
    - _Requirements: 7.1_

- [x] 10. Extend `order-service` consumer to handle `KitchenTicketAccepted` and `KitchenTicketRejected`
  - In `services/order-service/src/order_service/consumer.py`:
  - [x] 10.1 Add `handle_kitchen_ticket_accepted()` async handler
  ...
  - [x] 10.2 Add `handle_kitchen_ticket_rejected()` async handler following the same pattern
    - Call `service.cancel_order(order_id)` instead
    - _Requirements: 7.1, 7.3, 7.4, 7.5, 9.2, 9.3, 9.4_

  - [x] 10.3 Declare and bind two new durable queues in `consume()` setup
...
- [x] 11. Checkpoint — Ensure all order-service unit and property tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [x] 12. Add Alembic migration to extend `order_status` PostgreSQL enum
...
- [x] 13. Document `KitchenTicketAccepted` and `KitchenTicketRejected` event contracts
